...
_WRITE_MANIFEST = {
    "schema_version": 1,
    "steps": [
        {"id": "s1", "tool": "document.create", "input": {"type": "task", "title": "{{title}}", "body": "body"}},
    ],
}
...
def cipher() -> LocalEnvelopeCipher:
    return LocalEnvelopeCipher.from_base64_key(__import__("base64").urlsafe_b64encode(b"0" * 32).decode())
...
async def test_write_step_with_expired_approval_rejected(
    ...
    async with async_sessionmaker(engine, expire_on_commit=False)() as session:
        await session.execute(
            text("UPDATE execution_approvals SET expires_at = now() - interval '1 second' WHERE document_id = :doc"),
            {"doc": doc_id},
        )
        await session.commit()
    token = signer.sign(
...
async def test_stuck_reserved_row_returns_in_progress(
    ...
    async with async_sessionmaker(engine, expire_on_commit=False)() as session:
        await session.execute(
            text(
                "insert into sandbox_tool_calls (run_id, user_id, step_id, tool_id, "
...
async def test_replayed_failed_step_reports_failure(
    ...
    error_data = {"error_code": "VALIDATION_FAILED", "error_message": "Invalid document type."}
    async with async_sessionmaker(engine, expire_on_commit=False)() as session:
        await session.execute(
            text(
                "insert into sandbox_tool_calls (run_id, user_id, step_id, tool_id, "
            ...
            {"rid": run_id, "uid": user_id, "ct": cipher.encrypt(json.dumps(error_data))},
        )
        await session.commit()
...
async def test_crash_after_reservation_finalizes_as_failed(
    ...
    async with async_sessionmaker(engine, expire_on_commit=False)() as session:
        status = await session.scalar(
            text("SELECT result_status FROM sandbox_tool_calls WHERE run_id = :rid AND step_id = 's1'"),
            {"rid": run_id},
        )
    assert status == "failed", (
        f"Expected 'failed' after crash, got {status!r} — try/finally did not persist the failure state, row is stuck."
    )
...
async def test_user_b_cannot_poison_user_a_step_via_broker_read(
    ...
    async with async_sessionmaker(engine, expire_on_commit=False)() as session:
        await session.execute(
            ...
            {"rid": run_id, "uid": user_b, "ct": cipher.encrypt(json.dumps({"evil": True}))},
        )
        await session.commit()
