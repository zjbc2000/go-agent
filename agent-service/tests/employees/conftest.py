"""Fixtures for employee repository / approval / API tests against local Postgres.

Mirrors ``tests/planning/conftest.py``: every user-scoped transaction runs as the
``authenticated`` role with ``request.jwt.claims`` from the ``RequestContext``, so
RLS evaluates against the caller. ``employees.user_id`` references ``auth.users(id)``,
so each synthetic test user is provisioned a matching row first.
"""
...
_EMPLOYEE_TABLES = "public.employee_approvals, public.employees"

# The test suite TRUNCATEs employee tables before each test. That is safe ONLY against
# an isolated test database. If TEST_DATABASE_URL is not set and the default points at
# the dev database (`/postgres`), refuse to run.
...
async def _provision_user(db_session: AsyncSession, user_id: uuid.UUID) -> None:
    """Insert a minimal auth.users row so the ``employees.user_id`` FK is satisfied."""
...
async def _clean_employee_tables(engine: AsyncEngine) -> None:
    """Truncate employee tables before each test for a deterministic starting state."""
    async with async_sessionmaker(engine, expire_on_commit=False)() as session:
        await session.execute(text(f"truncate table {_EMPLOYEE_TABLES} cascade"))
...
@pytest.fixture
def repository(engine: AsyncEngine, cipher: LocalEnvelopeCipher) -> EmployeeRepository:
    return EmployeeRepository(session_factory=async_sessionmaker(engine, expire_on_commit=False), cipher=cipher)
...
@pytest.fixture
def service(
    repository: EmployeeRepository, chat_repository: ChatRepository, cipher: LocalEnvelopeCipher
) -> EmployeeService:
    """An employee service wired to the test DB and a deterministic chat service."""
    from app.agent.graph import build_assistant_graph

    documents = DocumentRepository(session_factory=chat_repository._session_factory, cipher=cipher)
    graph = build_assistant_graph(DeterministicProvider(delay_seconds=0.0), documents, repository)
    chat = ChatService(chat_repository, DeterministicProvider(), timedelta(days=7), graph=graph)
    return EmployeeService(repository=repository, cipher=cipher, chat=chat)
